...
@click.option(
    ...
    default="cli",
    show_default=True,
    help="Channel identifier sent to the Platform (e.g. cli, wechat).",
)
@click.option(
    "--force",
    is_flag=True,
    default=False,
    help="Overwrite existing binding without prompting.",
)
def bind(channel: str, force: bool) -> None:
    """Bind this Agent instance to an edu-platform user via credential code.

    The credential code is a one-time 8-character code generated by the Platform
    during student registration.  Run this once — the resulting channel_token is
    stored in ~/.edu_agent/identity.json and refreshed automatically on expiry.
    """
    import uuid
    ...
    platform_bind_key = (settings.platform_bind_key or "").strip()
    if not platform_bind_key:
        click.echo(
            click.style(
                "[错误] platform_bind_key 未配置。"
                "请在 edu_agent.yaml 中设置 platform_bind_key 或通过环境变量 PLATFORM_BIND_KEY 提供。",
                fg="red",
            ),
            err=True,
        )
    ...
    if existing and not force:
        click.echo(
            click.style(
                f"已存在绑定：platform_user_id={existing.get('platform_user_id')}  "
                f"agent_user_id={existing.get('agent_user_id')}",
                fg="yellow",
            )
        )
        click.confirm("是否覆盖现有绑定？", abort=True)

    # Resolve or generate agent_user_id
    agent_user_id = (settings.agent_user_id or "").strip()
    if not agent_user_id:
        if existing and existing.get("agent_user_id"):
            agent_user_id = existing["agent_user_id"]
        else:
            agent_user_id = str(uuid.uuid4())
        click.echo(click.style(f"使用 agent_user_id: {agent_user_id}", dim=True))

    code = click.prompt("请输入 8 位凭证码").strip()

    async def _do_bind() -> dict:
        challenge = await bind_client.bind_start(
            settings.platform_base_url, platform_bind_key, code
        )
        return await bind_client.bind_complete(
    ...
    try:
        result = asyncio.run(_do_bind())
    except bind_client.BindConflictError as exc:
        click.echo(
            click.style(
                "[错误] 无法绑定：当前 agent_user_id 已在平台绑定到其他账号。\n"
                "  若要用新注册账号的凭证码绑定，请在 edu_agent.yaml 中更换 agent_user_id（或删除本地 identity 后重新生成），"
                f"再执行 edu bind。\n  详情：{exc}",
                fg="red",
            ),
            err=True,
        ...
        raise SystemExit(1) from exc
    except bind_client.BindInvalidError as exc:
        click.echo(
            click.style(
                f"[错误] 凭证或绑定步骤无效（含过期、已使用、challenge 失效等）：{exc}",
                fg="red",
            ),
            err=True,
        )
        raise SystemExit(1) from exc
    except bind_client.BindAuthError as exc:
        click.echo(click.style(f"[错误] API Key 校验失败：{exc}", fg="red"), err=True)
        raise SystemExit(1) from exc
    except bind_client.BindRateLimitedError as exc:
        click.echo(click.style(f"[错误] 请求过于频繁，请稍后重试：{exc}", fg="red"), err=True)
        raise SystemExit(1) from exc
    except httpx.RequestError as exc:
        base = settings.platform_base_url.rstrip("/")
        if exc.request is not None:
            url_hint = f"  失败的请求 URL：{exc.request.url}"
        else:
            url_hint = (
                f"  绑定会依次请求：POST {base}/api/v1/bind/start\n"
                f"               POST {base}/api/v1/bind/complete"
            )
        click.echo(
            click.style(
                f"[错误] 绑定失败（网络连接或传输错误）：{exc}\n"
                f"{url_hint}\n"
                f"  platform_base_url（配置）：{settings.platform_base_url}",
                fg="red",
            ),
            err=True,
        )
        ...
        click.echo(click.style(f"[错误] 绑定失败：{exc}", fg="red"), err=True)
        raise SystemExit(1) from exc

    # Decode token_exp from JWT payload (no signature verification needed here)
    token_exp: int | None = None
    try:
        import base64

        parts = result["channel_token"].split(".")
        if len(parts) == 3:
    ...
    click.echo(
        click.style(
            f"绑定成功！platform_user_id={result['platform_user_id']}",
            fg="green",
        )
    )
