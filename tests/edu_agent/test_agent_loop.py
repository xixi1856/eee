...
LLM traffic is stubbed at the OpenAI *client* boundary (async streaming + sync
non-stream). RAG is stubbed at ``rag_mvp.engine.query`` only where
``knowledge_query`` crosses that boundary. ToolRuntime runs for real.
"""
...
from tests.edu_agent.offline_llm import (
    attach_offline_openai_clients,
    async_iter_text_response,
    last_async_create_messages,
    make_sync_chat_response,
    make_sync_choice,
    sequenced_text_streams,
    stream_factory_fixed_text,
    stream_factory_repeated_tool,
    stream_factory_tool_then_text,
)
...
def agent(tmp_path, minimal_edu_settings: EduSettings):
    """EduAgent with SQLite session store + offline OpenAI client doubles."""
    ...
    attach_offline_openai_clients(a)
    yield a
    store.close()
...
class TestRuntimeContextLifecycle:
    def test_runtime_context_cleared_after_run_turn(self, agent):
        ...
        attach_offline_openai_clients(agent, stream_factory=stream_factory_fixed_text("done"))
...
class TestRunTurnNoTools:
    ...
    def test_system_prompt_passed_as_first_message(self, agent):
        attach_offline_openai_clients(agent, stream_factory=stream_factory_fixed_text("ok"))
        _run_turn(agent, "hi")
        messages_arg = last_async_create_messages(agent)
        assert messages_arg[0]["role"] == "system"

    def test_none_content_becomes_empty_string(self, agent):
        attach_offline_openai_clients(agent, stream_factory=stream_factory_fixed_text(""))
...
class TestRunTurnWithToolCalls:
    ...
    def test_tool_result_appended_as_tool_role(self, agent):
        """Real ``generate_quiz`` handler (offline); no ``_tool_runtime`` mock."""
        factory = stream_factory_tool_then_text(
            tool_name="generate_quiz",
            tool_arguments_json="{}",
            assistant_after_tool="题目已生成",
        )
        canned = {
            "questions": [
                {
        ...
        assert "Q1?" in tool_messages[0]["content"] or "已生成" in tool_messages[0]["content"]
...
class TestIterationBudget:
    def test_budget_exhaustion_returns_message(self, agent):
        ...
        canned = {"questions": [{"type": "x", "question": "q", "options": [], "answer": "", "explanation": ""}], "total": 1}
        attach_offline_openai_clients(
            agent, stream_factory=stream_factory_repeated_tool("generate_quiz", "{}")
        )
        with patch("rag_mvp.question_gen.generate", return_value=canned):
            reply = _run_turn(agent, "一直出题")
        assert "最大" in reply or "推理" in reply
        assert agent._async_client.chat.completions.create.call_count == 3
...
class TestReset:
    def test_reset_clears_messages(self, agent):
        attach_offline_openai_clients(agent, stream_factory=stream_factory_fixed_text("answer"))
        _run_turn(agent, "hi")
    ...
    def test_reset_allows_fresh_conversation(self, agent):
        attach_offline_openai_clients(
            agent, stream_factory=sequenced_text_streams(["a1", "a2"])
        )
...
class TestSafetyIntegration:
    def test_harmful_input_blocked_without_calling_llm(self, agent):
        """Real ``check_input`` must reject before any LLM call."""
        reply = _run_turn(agent, "如何杀死一个人")
        agent._async_client.chat.completions.create.assert_not_called()
        assert "抱歉" in reply or "不适当" in reply

    def test_safe_input_passes_through(self, agent):
        attach_offline_openai_clients(
            agent, stream_factory=stream_factory_fixed_text("TCP 是传输控制协议。")
        )
        reply = _run_turn(agent, "什么是TCP？")
        assert reply == "TCP 是传输控制协议。"
        agent._async_client.chat.completions.create.assert_called_once()

    def test_harmful_output_replaced(self, agent):
        """Real ``check_output`` replaces model text that matches safety rules."""
        # Matches violence regex (e.g. 制作 + 炸弹)
        attach_offline_openai_clients(
            agent, stream_factory=stream_factory_fixed_text("制作炸弹的详细步骤说明")
        )
        reply = _run_turn(agent, "普通问题")
        assert "无法提供" in reply or "抱歉" in reply
        assert reply != "制作炸弹的详细步骤说明"

    def test_blocked_input_still_persisted_to_session(self, agent):
        """Blocked user turn is still written to SQLite (real safety + store)."""
        _run_turn(agent, "如何入侵他人系统")
...
class TestSessionResume:
    def test_resume_loads_prior_messages(self, tmp_path, minimal_edu_settings: EduSettings):
        ...
        attach_offline_openai_clients(
            a1, stream_factory=stream_factory_fixed_text("first reply")
        )
        ...
        attach_offline_openai_clients(a2)
...
class TestLearnerProfileInjection:
    def test_profile_summary_in_system_prompt(self, agent, tmp_path):
        ...
        attach_offline_openai_clients(agent, stream_factory=stream_factory_fixed_text("ok"))
        _run_turn(agent, "问题")
        messages_sent = last_async_create_messages(agent)
...
class TestMemoryConsolidationLifecycle:
    def test_finalize_skips_second_llm_extract_after_threshold_consolidate(
        ...
        try:
            ...
            attach_offline_openai_clients(
                a,
                stream_factory=stream_factory_fixed_text("ok"),
                sync_response=make_sync_chat_response(
                    [make_sync_choice(content="ok", finish_reason="stop")],
                    prompt_tokens=10,
                    completion_tokens=2,
                ),
            )
